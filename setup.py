...
DESCRIPTION = (
    "Wi-Fi Direct Session Manager, implementing a host AP daemon "
...
if os.environ.get("GITHUB_RUN_NUMBER") is not None:
    ...
    if (
        version_list_pypi
        or version_list_testpypi
        or os.environ.get("GITHUB_FORCE_RUN_NUMBER") is not None
    ):
        print("------------------------------------------------------------------")
        print("Using build number " + os.environ["GITHUB_RUN_NUMBER"])
        if version_list_pypi:
            print("Version list available in pypi: " + ", ".join(version_list_pypi))
...
setup(
    ...
    long_description=long_description[: long_description.find(END_OF_INTRODUCTION)]
    + EPILOGUE,
    ...
    zip_safe=False,
    install_requires=["python-daemon", "pyyaml"],
    python_requires=">3.5",
)
